  ...
  updateUser: async (req, res, next) => {
    try {
      if (req.body.matKhauMoi) {
        const existAdmin = await Admin.findById(req.body.id);
        if (!existAdmin) {
          throw new ApiError(StatusCodes.NOT_FOUND, "Người dùng không tồn tại");
        }
        if (
          !bcryptjs.compareSync(req.body.matKhauHienTai, existAdmin.matKhau)
        ) {
          throw new ApiError(
            StatusCodes.NOT_ACCEPTABLE,
        ...
        const matKhauMoi = bcryptjs.hashSync(req.body.matKhauMoi, 8);
